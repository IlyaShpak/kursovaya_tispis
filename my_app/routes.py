...
filename = os.path.abspath("/home/ilya/PycharmProjects/flaskProject3/my_app/xgb_models/model.sav")
...
def process_data():
    ...
    try:
        prediction = model.predict_proba(data).flatten().tolist()
        ...
        app.logger.info(f"Successful prediction for user {user_id}")
...
@app.route('/process_manual_data', methods=['POST'])
@login_required
def process_manual_data():
    img = None
    try:
        form_data = request.form.to_dict(flat=True)
        data = pd.DataFrame([form_data], dtype=float)
    except Exception as e:
        app.logger.error(f"Error converting form data: {e}")
